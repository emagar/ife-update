...
workdir <- c("d:/01/Dropbox/ifeSharedGE/data/")
setwd(workdir)
...
party.456789 <- c("PRI", "PAN", "PRI", "PVEM",  "PAN", "PRI", "PRI", "PAN", "PAN", "PRD", "PRI", "PAN", "PAN", "PRD", "PRI", "PAN", "PRD", "PRI")
party.456789 <- ifelse (party.456789=="PRI", 1, ifelse (party.456789=="PAN", 2, ifelse (party.456789=="PRD", 3, 4)))

# Read Ugalde et al's IFE's votes, only informative votes
all456789 <-read.csv("tmp456789.csv",  header=TRUE)
J <- 18

## ## subset votes up to Andrade, Alc�ntar, S�nchez's exit
## all456789 <- all456789[all456789$term<=7,]
## names.456789 <- names.456789[1:15]
## color.456789 <- color.456789[1:15]
## party.456789 <- party.456789[1:15]
## J <- 15

## subset votes up to C�rdova, Marv�n, Garc�aRmz entry
all456789 <- all456789[all456789$term<=8,]
J <- 15
names.456789 <- names.456789[1:J]
color.456789 <- color.456789[1:J]
party.456789 <- party.456789[1:J]
...
dim (all456789)
all456789 <- all456789[v.true==FALSE,]
dim (all456789)
...
#                 a  b  d  c  z  t  z  r  n  l  �  c  i  g  e  r  r  r
...
s <- 174 ## LA PRIMERA VENTANA EN QUE ENTRAN VALD�S NACIF Y BA�OS... SE ROMPE EL C�DIGO. COMPARARLO CON EL DE WOLDENBERG BONICA QUE SI FUNCIONA
for (s in 174:S){        # <= BIG FUNCTION STARTS (loop over 1081 windows)
	...
	v <- all456789[inicio[s]:final[s],1:15][,councilor.in==FALSE]; ## EXTRACT 30 VOTES EACH TIME
	v[v==0] <- NA; v[v==-1] <- 0    ## Version probit requiere 0s y 1s
...
#	partyPlacement <- apply( rbind (results[[1]]$BUGSoutput$sims.list$partyPos, results[[2]]$BUGSoutput$sims.list$partyPos), 2, median)
	...
	for (n in 1:15){
		if (length (which (councilors==names.456789[n]))==0) {
			x.location[n] <- NA
			x.precision[n] <- NA
		}
		else { x.location[n] <-  locs[which (councilors==names.456789[n])] }
	}
...
CouncilorIn[1,  all456789$date > 20071217] <- NA
CouncilorIn[2,  all456789$date > 20080814] <- NA
CouncilorIn[3,  all456789$date > 20101027] <- NA
CouncilorIn[4,  all456789$date > 20101027] <- NA
CouncilorIn[5,  all456789$date > 20080814] <- NA
CouncilorIn[6,  all456789$date > 20071217] <- NA
CouncilorIn[7,  all456789$date > 20080814] <- NA
CouncilorIn[8,  all456789$date > 20071217] <- NA
CouncilorIn[9,  all456789$date > 20101027] <- NA
CouncilorIn[10,  all456789$date < 20080215] <- NA
CouncilorIn[11,  all456789$date < 20080215] <- NA
CouncilorIn[12,  all456789$date < 20080215] <- NA
CouncilorIn[13,  all456789$date < 20080829] <- NA
CouncilorIn[14,  all456789$date < 20080829] <- NA
CouncilorIn[15,  all456789$date < 20080829] <- NA
#CouncilorIn[16,  all456789$date < 20111215] <- NA
#CouncilorIn[17,  all456789$date < 20111215] <- NA
#CouncilorIn[18,  all456789$date < 20111215] <- NA
