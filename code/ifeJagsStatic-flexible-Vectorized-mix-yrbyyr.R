...
# explore
with(vot, table(dunan=dunan, yr=yrn))
with(vot, table(term=term[dunan==0], yr=yrn[dunan==0]))
...
########################################################
## subset anchor votes so they can be placed at start ##
## of every year's time series as votes 1 and 2       ##
########################################################
anchor.votes <- vot[ anchors,]
vot <-          vot[-anchors,]
#head(vot)
...
##########################################################
## determine year's members and their parties from term ##
##########################################################
## sel <- c("c","d")
## sel <- c("4","6","7","8","9","a","b")
sel <- c("2","3")
sel <- sel[which(sel == vot$term[vot$t==t][1])]
...
# paste term anchors as year's votes 1 and 2
sel <- vot$term[vot$t==t][1] # year's term
rc <- rbind(anchor.votes[anchor.votes$term==sel, column.t], rc)
...
#                   , a = c(1,1.5,2)
                   , a = c(1,1,1) # ignorant about group sizes and their st
                     ))
...
############################
getwd()
post.samples <- vector(length=T, mode="list")
names(post.samples) <- paste0("yr", yr.by.yr$approx.yr)
#load("posterior-samples/not-in-git/posterior-chains-yrbyyr-statics-mix-2-3-items.RData")
post.samples[[t]] <- list(map.vote.indices=map.vote.indices,
                          map.member.indices=map.member.indices,
                          gelman.diag,
                          chains=chains)
summary(post.samples)
save(post.samples, file = "posterior-samples/not-in-git/posterior-chains-yrbyyr-statics-mix-2-3-items.RData")
